...
import type { WellnessPersistV1, WellnessPersistV2 } from "./wellnessTypes";
...
export function readWellnessPersist(): WellnessPersistV2 {
  ...
  try {
    ...
    const data = JSON.parse(raw) as WellnessPersistV1 | WellnessPersistV2;
    if (data.v === 2) {
...
export function writeWellnessPersist(snapshot: WellnessPersistV2): void {
  try {
